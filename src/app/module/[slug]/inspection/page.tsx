...
export default function InspectionPage() {
  ...
  const inspectionAssets: InventoryAsset[] | null = useMemo(() => {
    if (!rawInventoryAssets) {
      return null;
    }
    return rawInventoryAssets.filter(asset =>
        asset.status && typeof asset.status === 'string' && asset.status.trim().toLowerCase().includes('inspection')
    ).map((asset) => {
      ...
      <main className="flex flex-1 flex-col gap-4 p-4 md:gap-8 md:p-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
        ...
        <Card>
            <CardHeader>
                ...
                <CardDescription>A list of all assets due for inspection.</CardDescription>
            </CardHeader>
            <CardContent>
