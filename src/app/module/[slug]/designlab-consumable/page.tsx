...
export default function DesignLabConsumablePage() {
    const params = useParams();
    const slug = params.slug as string;
    const firestore = useFirestore();

    const designlabConsumableQuery = useMemoFirebase(() => {
        if (!firestore) return null;
        return collection(firestore, `modules/designlab_consumable/inventory_list`);
    }, [firestore]);

    const { data: designlabConsumables, isLoading } = useCollection<DesignLabConsumable>(designlabConsumableQuery);

    const moduleName = useMemo(() => {
        if (!slug) return "DesignLab Consumable";
        const baseName = getModuleNameFromSlug(slug);
        return `${baseName} - DesignLab Consumable`;
    }, [slug]);
    ...
    return (
        <div className="flex min-h-screen w-full flex-col">
            <main className="flex flex-1 flex-col gap-4 p-4 md:gap-8 md:p-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
                ...
                <Card>
                    <CardHeader>
                        <CardTitle>DesignLab Consumable Inventory</CardTitle>
                        <CardDescription>
                            ...
                            <Table>
                                ...
                                <TableBody>
                                    {isLoading ? (
                                        ...
                                        designlabConsumables.map((item) => (
                                            <TableRow key={item.id}>
                                                {tableHeaders.map(header => {
                                                    const fieldKey = headerToFieldMap[header];
                                                    let value = fieldKey ? item[fieldKey as keyof DesignLabConsumable] : '';
